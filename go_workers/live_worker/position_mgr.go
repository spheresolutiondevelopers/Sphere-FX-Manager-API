...
// GetOpenPositions returns all open positions for a given account.
func GetOpenPositions(accountID int64) ([]Position, error) {
	...
	`
	rows, err := db.GetDB().Query(query, accountID)
	if err != nil {
...
// UpdatePositionPrice updates the current price and P&L for a position.
func UpdatePositionPrice(positionID int64, currentPrice, pnl float64) error {
	query := `
		...
		WHERE id = @p3
	`
	_, err := db.GetDB().Exec(query, currentPrice, pnl, positionID)
	if err != nil {
...
// ClosePosition closes a position and updates the status.
func ClosePosition(positionID int64, closeReason string) error {
	...
	query := `
		...
		WHERE id = @p3
	`
	_, err := db.GetDB().Exec(query, now, closeReason, positionID)
	if err != nil {
